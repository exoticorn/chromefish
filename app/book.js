define(function() {
  ...
  var randomLow = new Uint32Array(781);
  var randomHigh = new Uint32Array(781);

  random64.split(/\s+/).forEach(function(v, i) {
    randomHigh[i] = parseInt(v.slice(0, 8), 16);
    randomLow[i] = parseInt(v.slice(8), 16);
  });

  var pieceTypes = {'p': 0, 'P': 1, 'n': 2, 'N': 3, 'b': 4, 'B': 5,
    'r': 6, 'R': 7, 'q': 8, 'Q': 9, 'k': 10, 'K': 11};

  var castleTypes = {'K': 0, 'Q': 1, 'k': 2, 'q': 3};

  function getKey(fen) {
    var parts = fen.split(' ');
    var keyHi = 0, keyLo = 0;
    var epSquares = {};
    parts[0].split('/').forEach(function(line, row) {
      var file = 0;
      for(var i = 0; i < line.length; ++i) {
        ...
        if(c < 58) {
          ...
          c = line.charAt(i);
          var offset = 64 * pieceTypes[c] + 8 * (7 - row) + file;
          keyHi ^= randomHigh[offset];
          keyLo ^= randomLow[offset];
          if((c === 'p' && parts[1] === 'b') || (c === 'P' && parts[1] === 'w')) {
            var r = c === 'p' ? 7 - row : 9 - row;
            if(file > 0) {
              epSquares[String.fromCharCode(96 + file) + r] = true;
            }
            if(file < 7) {
              epSquares[String.fromCharCode(98 + file) + r] = true;
            }
          }
          ++file;
        }
    ...
    var castleString = parts[2];
    for(var i = 0; i < castleString.length; ++i) {
      var offset = castleTypes[castleString.charAt(i)];
      if(offset !== undefined) {
        offset += 768;
        keyHi ^= randomHigh[offset];
        keyLo ^= randomLow[offset];
    ...
    if(epSquares[parts[3]]) {
      offset = 772 + parts[3].charCodeAt(0) - 97;
      keyHi ^= randomHigh[offset];
      keyLo ^= randomLow[offset];
    }
    if(parts[1] === 'w') {
  ...
  function testKey(fen, key) {
    var k = getKey(fen);
    k = ('0000000' + k.hi.toString(16)).slice(-8) +
      ('0000000' + k.lo.toString(16)).slice(-8);
    if(k !== key) {
      console.log('Failed: ' + fen + ' should be ' + key + ' but was ' + k);
    }
  }

  testKey('rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1', '463b96181691fc9c');
  testKey('rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq e3 0 1', '823c9b50fd114196');
  testKey('rnbqkbnr/ppp1pppp/8/3p4/4P3/8/PPPP1PPP/RNBQKBNR w KQkq d6 0 2', '0756b94461c50fb0');
  testKey('rnbqkbnr/ppp1pppp/8/3pP3/8/8/PPPP1PPP/RNBQKBNR b KQkq - 0 2', '662fafb965db29d4');
  testKey('rnbqkbnr/ppp1p1pp/8/3pPp2/8/8/PPPP1PPP/RNBQKBNR w KQkq f6 0 3', '22a48b5a8e47ff78');
  testKey('rnbqkbnr/ppp1p1pp/8/3pPp2/8/8/PPPPKPPP/RNBQ1BNR b kq - 0 3', '652a607ca3f242c1');
  testKey('rnbq1bnr/ppp1pkpp/8/3pPp2/8/8/PPPPKPPP/RNBQ1BNR w - - 0 4', '00fdd303c946bdd9');
